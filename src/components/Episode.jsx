...
export default function Episode({
  ...
  useEffect(() => {
    let dataToSet = Array.isArray(toshoEps) ? toshoEps : []

    // Extract seasons from torrents (for "All" button)
    if (all && setAvailableSeasons && dataToSet.length > 0) {
      const seasons = new Set(['all']);
      dataToSet.forEach(torrent => {
        const title = torrent?.title || '';
        // Match patterns like "S2", "Season 2", "2nd Season", etc.
    ...
    if (data?.selectedSeason && data.selectedSeason !== 'all') {
      dataToSet = dataToSet.filter((torrent) => {
        const title = torrent?.title || '';
        const seasonPattern = new RegExp(`(?:S|Season\\s*)${data.selectedSeason}`, 'i');
    ...
    dataToSet.sort((a, b) => b.seeders - a.seeders)
    setTorrentData(dataToSet)
  }, [dualAudio, toshoEps, data?.selectedSeason, all, setAvailableSeasons])
            ...
            <div>
              <p className="flex gap-x-2 font-space-mono text-lg font-medium opacity-90">
                <span className="flex items-center gap-2 text-gray-400">
                  {t('animePage.all')} <MagnifyingGlassIcon />
                </span>
                | {anime.romaji}
              </p>
            </div>
          ...
          >
                ...
                <p className="flex items-center gap-2 font-space-mono text-lg font-medium opacity-100">
                  {episodeNumber <= progress && (
                    <Tooltip content="Watched">
                      <span className="h-2 min-h-2 w-2 min-w-2 rounded-full bg-green-500"></span>
                    </Tooltip>
                  )}
            ...
            <div className="flex w-fit gap-x-2 text-xs">
              ...
              {data.airdate && (
                ...
                >
                  {format(new Date(data.airdate), 'dd MMM yyyy', { locale: tr })}
                </p>
              )}
